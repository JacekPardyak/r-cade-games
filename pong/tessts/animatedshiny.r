...
ui<-fluidPage(
  ...
  sidebarLayout(
    ...
    sidebarPanel(
      ...
      selectInput("fixed","System type",choices=c("Fixed","Random")),
    ...
    mainPanel(
      ...
      # tab layout
      tabsetPanel(
        tabPanel("Pong",plotOutput('pongPlot')),
...
server<-function(input,output){
  ...
  waits$dx = 1 #(10 + runif(1)) * (-1) ^ sample(c(0, 1), 1)
  waits$dy = 1 #(10 + runif(1)) * (-1) ^ sample(c(0, 1), 1)
